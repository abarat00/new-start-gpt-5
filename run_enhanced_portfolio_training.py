...
def get_profile_config(args):
    ...
    profiles = {
        'active': {
            ...
            # Parametri per train()
            'train': {
                'lr_actor': 1e-5,  # Learning rate più alto
                'lr_critic': 2e-4,
        ...
        'longterm': {
            ...
            # Parametri per train()
            'train': {
                'lr_actor': 5e-6,  # Learning rate più basso
                'lr_critic': 1e-4,
                'weight_decay_actor': 1e-6,
                'weight_decay_critic': 1e-5,
                'decay_rate': 5e-7,
        ...
        'momentum': {
            ...
            # Parametri per train()
            'train': {
                'lr_actor': 8e-6,  # Learning rate medio
                'lr_critic': 1.5e-4,
                'weight_decay_actor': 1e-6,
                'weight_decay_critic': 1e-5,
                'decay_rate': 5e-7,
...
def main(args):
    ...
    # Run the actual training with all enhancements configured
    # Nel main(), quando chiami agent.train():
    """""
    results = agent.train(
        env=env,
        total_episodes=args.episodes,
        tau_actor=0.01,
        tau_critic=0.01,
        lr_actor=profile_config.get('lr_actor', 5e-6),           # Usa i valori dal profilo
        lr_critic=profile_config.get('lr_critic', 1e-4),         # Usa i valori dal profilo
        weight_decay_actor=1e-6,
        weight_decay_critic=1e-5,
        total_steps=3000,
        weights=f'{output_dir}\\weights\\',
        ...
        if last_model:
            ...
            if args.use_backtest_framework:
                ...
                while not done:
                    ...
                    # Log prima dell'azione
                    if step_counter % 50 == 0 or np.any(np.abs(actions) > 0.1):  # Log ogni 20 step o per azioni significative
                        print(f"DEBUG - Step {step_counter}:")
                        print(f"\nStep {step_counter}:")
                        print(f"Posizioni correnti: {test_env.positions}")
                        print(f"Azioni: {actions}")
                        print(f"Portfolio value: ${test_env.get_portfolio_value():.2f}")
                        print(f"  Max action magnitude: {np.max(np.abs(actions))}")
                        print(f"  Min action magnitude: {np.min(np.abs(actions))}")
                        print(f"  Portfolio value: {test_env.get_portfolio_value():.2f}")
                        print(f"  Cash: {test_env.cash:.2f}")
                        print(f"  Positions sum: {np.sum(np.abs(test_env.positions)):.4f}")
